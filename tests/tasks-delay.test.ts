...
describe('formatCascadeSummaryMessage — AC-DL-3', () => {
  it('pluralizes both counts above 1', () => {
    expect(formatCascadeSummaryMessage(3, 5)).toBe(
      '3 substages shifted, 5 material order-by dates moved'
    )
  })

  it('singularizes substage when count is 1', () => {
    expect(formatCascadeSummaryMessage(1, 5)).toBe(
      '1 substage shifted, 5 material order-by dates moved'
    )
  })

  it('singularizes material date when count is 1', () => {
    expect(formatCascadeSummaryMessage(3, 1)).toBe(
      '3 substages shifted, 1 material order-by date moved'
    )
  })

  it('renders zero counts honestly', () => {
    expect(formatCascadeSummaryMessage(1, 0)).toBe(
      '1 substage shifted, 0 material order-by dates moved'
    )
  })

  it('handles zero substages (defensive — cascade always returns at least the trigger)', () => {
    expect(formatCascadeSummaryMessage(0, 0)).toBe(
      '0 substages shifted, 0 material order-by dates moved'
    )
  })
})
